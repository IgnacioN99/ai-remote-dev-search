...
describe("htmlFetch retry/backoff", () => {
  test("retries a 429 and succeeds on the next attempt", async () => {
    ...
    const html = await htmlFetch("https://weworkremotely.com/remote-jobs/search");
  ...
  test("returns the documented empty string on 404 without retrying", async () => {
    ...
    const html = await htmlFetch("https://weworkremotely.com/remote-jobs/does-not-exist");
    expect(html).toBe("");
  ...
  test("gives up after the initial attempt plus six retries on persistent 5xx", async () => {
    ...
    await expect(htmlFetch("https://weworkremotely.com/remote-jobs/search")).rejects.toThrow(/500/);
