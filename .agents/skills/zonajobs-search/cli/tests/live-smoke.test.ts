...
// Live smoke tests against ZonaJobs Argentina's real API. Keeps volume low —
// three requests: one search + one detail on the first hit. The test query is
// the fork's canonical Argentine market search.
describe("live search", () => {
  test('search -q "desarrollador" returns ≥1 real result', async () => {
    ...
    for (const r of body.results) {
      ...
      expect(r.url).toMatch(/^https:\/\/www\.zonajobs\.com\.ar\/empleos\//);
    }
  }, 60000);
