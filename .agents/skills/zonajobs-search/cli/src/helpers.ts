// Data source: the public JSON API behind ZonaJobs Argentina's SPA
// (www.zonajobs.com.ar). The SPA shell is a client-rendered React app, so the
...
//
// Both require the "x-site-id" header ("ZJAR" = ZonaJobs Argentina; ZonaJobs
// uses "ZJAR" against the same backend). No authentication, no API key.

export const SITE = "https://www.zonajobs.com.ar"
export const X_SITE_ID = "ZJAR"
...
/**
 * Slugify the way ZonaJobs's SPA does (formatNameToStringId): lowercase, strip
 ...
 * ZonaJobs/ZonaJobs posting URLs always end with "<id>.html", so prefer that
...
/**
 * Map a --jobage (days) to the portal's posting-age filter value. ZonaJobs's
