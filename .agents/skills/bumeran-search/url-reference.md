# Bumeran Argentina URL Reference

Public, unauthenticated JSON API behind Bumeran Argentina's SPA
(www.bumeran.com.ar). ZonaJobs (www.zonajobs.com.ar) uses the **same backend**
with a different `x-site-id` (`ZJAR`). The HTML pages are a client-rendered
...
(`BMAR` for Bumeran Argentina). Sending `Origin`/`Referer` matching the site
...
```
POST https://www.bumeran.com.ar/api/avisos/searchV2?pageSize=<n>&page=<p>
```
  ...
  "content": [
    {
      ...
      "fechaHoraPublicacion": "05-08-2026 10:52:44",
...
```
GET https://www.bumeran.com.ar/api/candidates/fichaAvisoNormalizada/<id>
```
...
```json
{
  "aviso": {
    ...
    "nivelLaboral": { "id": 2, "nombre": "Junior" },
...
Province filter values come from `GET https://www.bumeran.com.ar/api/provincias/1`
...
- When the site changes this SPA, re-discover the endpoints from
  `https://www.bumeran.com.ar/candidate/static/js/main.<hash>.js` — the API
  paths and the `x-site-id` default (`BMAR`) live in the webpack bundle.
