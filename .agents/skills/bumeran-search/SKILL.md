...
# Bumeran Argentina Search Skill

Search live job listings from **Bumeran Argentina** (www.bumeran.com.ar), one of
the largest job boards in the country, via its public web API. No authentication,
no API key, and **zero runtime dependencies** — it runs with just `bun`.

## ⚠️ Personal use only

This uses Bumeran's public job listings through its web API; automated access is
...
```bash
bun run .agents/skills/bumeran-search/cli/src/cli.ts search --query "<keywords>" [flags]
```
...
`id` is the numeric job ID from `search` results (e.g. `1118391216`). You may
also pass the full `/empleos/...-<id>.html` URL. Returns the full description
with paragraph breaks preserved, plus modality, employment type, contract type,
and seniority.
...
bun run .agents/skills/bumeran-search/cli/src/cli.ts search -q "desarrollador" --format table

# Backend developer roles, Buenos Aires province, last 7 days
bun run .agents/skills/bumeran-search/cli/src/cli.ts search -q "desarrollador backend" -l "Buenos Aires" --jobage 7 --format table

# Ruby on Rails roles in Capital Federal
bun run .agents/skills/bumeran-search/cli/src/cli.ts search -q "ruby on rails" -l "Capital Federal" --format json

# React roles in Córdoba, first 10
bun run .agents/skills/bumeran-search/cli/src/cli.ts search -q "react" -l "Córdoba" --limit 10 --format table

# Full details for a specific job
bun run .agents/skills/bumeran-search/cli/src/cli.ts detail 1118391216 --format plain
```
...
- Data is from Bumeran's public web API (`POST /api/avisos/searchV2`,
  `GET /api/candidates/fichaAvisoNormalizada/<id>`); the page HTML is a
  client-rendered shell with no listings.
- The API requires an `x-site-id` header; Bumeran Argentina uses `BMAR`
  (ZonaJobs uses `ZJAR` against the same backend). This is a public site
...
- Bumeran may rate-limit; the CLI retries 429/5xx with exponential backoff.
  Keep volume low (see personal-use note above).
- Job IDs are numeric (e.g. `1118391216`) — pass them as-is to `detail`.
