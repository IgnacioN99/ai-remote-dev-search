...
describe("Remotive apiGet retry and error handling", () => {
  ...
  test("throws after exhausting retries on persistent 500", async () => {
    ...
    await expect(apiGet()).rejects.toThrow(/request failed/i);
  ...
  test("fails fast on network connection error without infinite retrying", async () => {
    ...
    await expect(apiGet()).rejects.toThrow(/could not reach the Remotive API/i);
